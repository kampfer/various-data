...
proxies = (
    {"http": "http://127.0.0.1:33210", "https": "http://127.0.0.1:33210"}
    if useProxy
    else None
)
...
def crawlArtstationProject(id, session):
    ...
    r = session.get(
        f"https://www.artstation.com/projects/{id}.json",
        proxies=proxies,
        headers=headers,
    )
    ...
    for index, asset in enumerate(data["assets"]):
        ...
        if assetType == "image" or assetType == "cover":
            crawlImage(
                asset["image_url"], os.path.join(dirPath, basename(asset["image_url"]))
            )
            ...
            crawlMP4(source, os.path.join(dirPath, basename(source)), session)
...
def crawlArtstationCollection(id, s):
    r = s.get(
        f"https://www.artstation.com/collections/{id}/projects.json?collection_id={id}",
        proxies=proxies,
    )
    data = json.loads(r.text)
