...
class SinaNews7x24DB:
  ...
  def recoverFromJson(self, jsonFile):
    ...
    for item in data:
      ...
      # 新闻
      sina_id = item["id"]
      create_time = int(time.mktime(time.strptime(item["create_time"], "%Y-%m-%d %H:%M:%S")))
  ...
  def updateNewsSignificance(self, id, significance):
    ...
    cur.execute("UPDATE sina_news_7x24 SET significance=? WHERE id=?", (significance, id))
    self.conn.commit()
  ...
  def getNews(self, page, pageSize, keyword="", sort=0, significance=0, startTime=None, endTime=None):
    sql = f'''
    SELECT * FROM sina_news_7x24
    WHERE content LIKE :keyword 
    { "AND create_time >= :startTime AND create_time <= :endTime" if (startTime != None and endTime != None) else '' }
    { "AND significance=:significance" if significance != None else '' }
